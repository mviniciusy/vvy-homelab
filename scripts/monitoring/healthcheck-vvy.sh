...
# 3. nvidia-fancontrol — verificar se está ativo
FAN_STATUS=$(ssh "$HOST" 'systemctl is-active nvidia-fancontrol.service 2>/dev/null')
if [ "$FAN_STATUS" != "active" ]; then
    ALERTS="${ALERTS}\n[ALERTA] nvidia-fancontrol.service NAO esta ativo (status: ${FAN_STATUS}). Fans GPU podem estar em 0%."
fi
...
# 5. GPU temperatura
GPU_TEMP=$(ssh "$HOST" 'nvidia-smi --query-gpu=temperature.gpu --format=csv,noheader,nounits 2>/dev/null | head -1')
if [ -n "$GPU_TEMP" ] && [ "$GPU_TEMP" -gt 85 ]; then
    ALERTS="${ALERTS}\n[ALERTA] GPU temperatura: ${GPU_TEMP}C (threshold: 85C)"
fi
...
if [ -z "$ALERTS" ]; then
    echo "OK — vvy saudavel (load: ${LOAD}, GPU: ${GPU_TEMP}C, fan: ${FAN_STATUS}, heartbeat restarts: ${RESTART_COUNT})"
    exit 0
else
